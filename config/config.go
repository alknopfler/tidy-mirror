...
func NewConfig(configPath string, kubeconfig string) (MirrorConfig, error) {
	...
	if configPath == "" {
		return config, fmt.Errorf(color.InRed("configFile param is empty"), "")
	}
	...
	if kubeconfig == "" {
		return config, fmt.Errorf(color.InRed("Kubeconfig param is empty"), "")
	}
	...
	config.RegistryOLMSourceIndex += strings.Join(strings.Split(config.RegistryOCPRelease, ".")[:2], ".")
