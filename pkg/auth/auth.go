...
func NewZTPAuth(kubeconfig string) *MirrorAuth {
	return &MirrorAuth{
		KubeConfig: kubeconfig,
	}
}

func (z *MirrorAuth) GetProjectAuth() *projectv1.ProjectV1Client {
...
func (z *MirrorAuth) GetRouteAuth() *routev1.RouteV1Client {
	config, err := clientcmd.BuildConfigFromFlags("", z.KubeConfig)
	if err != nil {
		log.Fatalf(color.InRed("[ERROR] error reading kubeconfig to get clientset: %s"), err.Error())
	}
...
func (z *MirrorAuth) GetOlmAuth() *olmv1.OperatorsV1alpha1Client {
	config, err := clientcmd.BuildConfigFromFlags("", z.KubeConfig)
	if err != nil {
		log.Fatalf(color.InRed("[ERROR] error reading kubeconfig to get clientset: %s"), err.Error())
	}
...
func (z *MirrorAuth) GetAuth() *kubernetes.Clientset {
	config, err := clientcmd.BuildConfigFromFlags("", z.KubeConfig)
	if err != nil {
		log.Fatalf(color.InRed("[ERROR] error reading kubeconfig to get clientset: %s"), err.Error())
	}
...
func (z *MirrorAuth) GetAuthWithGeneric() dynamic.Interface {
